...
export async function generateVideoToken(formData) {
    ...
    try {
      ...
      if (!appointment.videoSessionId) {
        throw new Error("Video session is not initialized for this appointment");
      }
      ...
      // Only allow token generation within 30 minutes before start or anytime during/after start
      if (minutesUntilStart > 30) {
        throw new Error("The call will be available 30 minutes before the scheduled time");
      }
  
      const appointmentEndTime = new Date(appointment.endTime);
      const expirationTime = Math.floor(appointmentEndTime.getTime() / 1000) + 60 * 60; // +1 hour buffer
